...
function render(route) {
  ...
  if (route === '/') {
    renderHome(root, tools, (destination) => router.navigate(destination));
  } else if (route === '/timer') {
    renderTimer(root, () => router.navigate('/'));
  } else if (route === '/duvido') {
    renderDuvido(root, () => router.navigate('/'), () => router.navigate('/duvido/cards'));
  } else if (route === '/duvido/cards') {
    renderCardManager(root, () => router.navigate('/duvido'));
  } else {
    renderToolPlaceholder(root, findToolByRoute(route), () => router.navigate('/'));
  }
  root.focus();
}

const router = new Router({ onRouteChange: render });
